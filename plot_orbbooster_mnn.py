...
def mnn_matcher(descriptors0, descriptors1, topk=-1):
    ...
    topk_desc_dist = desc_dist[min_ids_dist]
...
def boost_features(keypoints, descriptors, booster, im_shape):
    keypoints = normalize_keypoints(keypoints, im_shape)
    ...
    # boost features
    out = booster(descriptors, keypoints)
    # Dim transfer: 256 -> 32
...
def main(args):
    ...
    # Load Booster
    booster = FeatureBooster(config[args.descriptor]).eval().to(args.device)
    model_path = 'weights/' + args.descriptor + '.pth'
    booster.load_state_dict(torch.load(model_path))
    ...
    gray0_shape = gray0.shape
    keypoints0, descriptors0, unnorm_kpts0 = extract_featrues(gray0, extractor)
    keypoints0, descriptors0 = boost_features(keypoints0, descriptors0, booster, gray0_shape)
    ...
    for idx in range(2, 7):
        ...
        gray1_shape = gray1.shape
        keypoints1, descriptors1, unnorm_kpts1 = extract_featrues(gray1, extractor)
        keypoints1, descriptors1 = boost_features(keypoints1, descriptors1, booster, gray1_shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--descriptor', type=str, default='ORB_Boost_B', choices=['ORB_Boost_B', 'SuperPoint_Boost_F', 'SuperPoint_Boost_B'])
    ...
    parser.add_argument('--matcher', type=str, default='MNN', choices=['MNN', 'SuperGlue'])
