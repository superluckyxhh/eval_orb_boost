...
def eval_hpatches(args):
    ...
    # Load Booster
    booster = FeatureBooster(config['ORB_Boost_B']).eval().to(args.device)
    model_path = '/home/xiaohunan/WorkSpace/MyCode/test_featurebooster/weights/ORB_Boost_B.pth'
    ...
    for scene in tqdm(scenes[::-1], total=len(scenes)):
        ...
        keypoints0, descriptors0 = boost_features(keypoints0, descriptors0, booster, im0.shape)
        ...
        for idx in range(2, 7):
            ...
            keypoints1, descriptors1 = boost_features(keypoints1, descriptors1, booster, im1.shape)
...
def main(args):
    ...
    dump_name = f'orb+booster_mnn_topK_{args.topk_match}_{args.auc_threshold}.json'
    dumps_save_path = osp.join(args.json_save_root, dump_name) 
    
    with open(dumps_save_path, 'w') as f:
        f.write(json.dumps(dumps, indent=4))
    np.save('eval_dumps/npy/orb_boost_mnn_auc_i.npy', aucs_i)
    np.save('eval_dumps/npy/orb_boost_mnn_auc_v.npy', aucs_v)
    np.save('eval_dumps/npy/orb_boost_mnn_aucs.npy', aucs)
